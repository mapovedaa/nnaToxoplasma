...
void Shakingtree::minimizeBasic()
{
	...
	double s = getScore(Datatype::TRAIN, batch_size);
	...
	double news = getScore(Datatype::TRAIN, batch_size);
...
void Shakingtree::minimizeBasicLarger()
{
	...
	//evaluate the new score
	double new_s = getScore(Datatype::TRAIN, batch_size);
...
void Shakingtree::minimizeComplex()
{
	...
	size_t EVALSIZE = 100;
	srand(_total_iter);
	double score = getScore(Datatype::TRAIN, EVALSIZE);
	...
	//evaluate score
	srand(_total_iter);
	double delta_score = getScore(Datatype::TRAIN, EVALSIZE) - score;
